...
export async function POST(req: Request) {
  try {
    ...
    await prisma.excludedFoodUser.deleteMany({
      where: { userId }
    });

    // Insertar nuevos registros
    const createdRecords = await prisma.excludedFoodUser.createMany({
...
export async function GET(req: Request) {
  try {
    ...
    const includedFoods = await prisma.excludedFoodUser.findMany({
      where: { userId },
    ...
    console.error('Error al obtener alimentos a excluir:', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
